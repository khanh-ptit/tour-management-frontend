...
import {
  computedDH,
  computedDHReceiver,
  computeWhenDhRatchet,
} from "../../../services/e2e/computeDh";
import {
  buf2hex,
  deriveMessageKey,
  deriveRootAndChainKeys,
  HMAC,
} from "../../../services/e2e/hkdf";
import { openDB } from "idb";
import {
  decryptMsg,
  encryptMsg,
  hexToBuf,
} from "../../../services/e2e/EncryptAndDecrypt";
import { computeDHFromSenderEK } from "../../../services/e2e/computeDhReceiver";
import {
  createBKKey,
  decodeLastestEBKtoBK,
  decryptByBKKey,
  encryptByBKKey,
} from "../../../services/e2e/Backup";
...
import {
  ...
  loadEphemeralKey,
  loadMessages,
} from "../../../services/e2e/ChatScreenUtils";
...
function ChatAdmin() {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        const filtered = res.data.result.filter((u) => u.username !== username);
  ...
  const handleVerifyPIN = async () => {
    const ebkLastest = await axios.get(`http://localhost:8080/getLastestEbk`, {
      headers: { Authorization: `Bearer ${localStorage.getItem("jtoken")}` },
    });
    try {
      const { bk, bkVersion } = await decodeLastestEBKtoBK(
        ebkLastest.data.result,
        pinInput
      ); // thử giải mã EBK
      const db = await initDB();

      await db.put("backup_keys", {
        bk: buf2hex(bk),
        bkVersion: ebkLastest.data.result.bkVersion,
        username: ebkLastest.data.result.user.username,
      });
  ...
  useEffect(() => {
    const checkBackup = async () => {
      ...
      const res = await axios.get(
        `http://localhost:8080/check-if-exist-any-backup-key?username=${username}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("jtoken")}`,
          },
    ...
    const subscription = stompClient.subscribe(
      ...
      (mes) => {
        (async () => {
          ...
          const res = await axios.post(
            `http://localhost:8080/updateStatusMessage?id=${msg.id}`,
            {},
            {
              headers: {
                Authorization: `Bearer ${localStorage.getItem("jtoken")}`,
  ...
  const openChat = async (receiver) => {
    ...
    // ===== CASE 2: Đã tồn tại conversation nhưng chưa có EK pair =====
      ...
      if (status === "not created") {
        ...
        const newIkm = await computeWhenDhRatchet(conversationId, ekPubKey);
  ...
  const handleSendMessage = async () => {
    ...
    const db = await initDB();
    const testSession = await db.get(
      "sessions",
    ...
    if (testSession.Ns >= 10) {
      ...
      if (otherEkPub.data.result !== "not yet joined") {
        ...
        const newIkm = await computeWhenDhRatchet(
          localStorage.getItem("currentConversationId"),
          otherEkPub.data.result
        );
        const oldSessions = await db.get(
          "sessions",
          localStorage.getItem("currentConversationId")
        );
        const oldRootKey = oldSessions.rootKey;
        //kết hợp với rootkey cũ
        const newKey = await deriveRootAndChainKeys(
          newIkm,
          hexToBuf(oldRootKey)
        );
        ...
        const localEphemeral = await loadEphemeralKey(
          localStorage.getItem("currentConversationId")
        );
    ...
    if (bkInfor) {
      const encryptInfo = await encryptByBKKey(
        inputMessage,
        hexToBuf(bkInfor.bk)
      );
      ...
      const backupRes = await axios.post(
        `http://localhost:8080/backup-message`,
        backupMsg,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("jtoken")}`,
    ...
    stompClient.send("/app/direct", {}, JSON.stringify(payloadSend));
  };
  return (
        ...
        <div className="otp_container">
          <div className="otp_sc">
            ...
            <button
              ...
              disabled={otpExpireTime === 0 || isOtpLoading}
            >
            ...
            <div className="chat-header">
              <div className="chat-header-left">
                <span className="chat-title">
                  {activeUser ? `💬 ${activeUser}` : "Chọn user"}
                </span>
              </div>
              ...
              <div className="chat-header-right">
                ...
                >
                  ...
                  <span className="e2ee-text">
                    Các tin nhắn được mã hóa đầu cuối
                  </span>
                </div>
              </div>
            </div>
